...
def client(tmp_path, monkeypatch) -> TestClient:
    ...
    # Course referencing the book only
    course_payload = {
        "id": "test-course",
        "title": "Test Course",
        "summary": "示範課程，引用現有題庫本。",
...
def test_course_detail_and_book_detail(client: TestClient):
    ...
    books = payload["books"]
    assert {b["id"] for b in books} == {"test-book"}
    referenced = books[0]
    assert referenced["itemCount"] == 1
    assert referenced["items"][0]["zh"].startswith("這是一句")

    book_detail = client.get("/cloud/courses/test-course/books/test-book")
    assert book_detail.status_code == 200
    book_payload = book_detail.json()
    assert book_payload["id"] == "test-book"
    assert book_payload["itemCount"] == 1
    assert book_payload["items"][0]["hints"][0]["category"] == "lexical"
